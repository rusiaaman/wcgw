...
from wcgw.client.bash_state.bash_state import execute_bash, render_terminal_output
from wcgw.client.tools import BashCommand, BashInteraction
...
class TestExecuteBash:
    ...
    def test_execute_bash_interaction(self, mock_bash_state):
        ...
        mock_bash_state.pending_output = ""
        mock_bash_state.update_cwd.return_value = "/test/path"
        mock_bash_state.update_repl_prompt.return_value = (
            False  # Important: Set this to False
        )
        mock_bash_state.prompt = "TEST_PROMPT>"
        mock_bash_state.ensure_env_and_bg_jobs.return_value = 0
        ...
        with patch("wcgw.client.tools.get_status", side_effect=mock_get_status):
            ...
            output, cost = execute_bash(
                self.mock_tokenizer, interaction, max_tokens=100, timeout_s=1
            )
